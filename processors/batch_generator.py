...
class DataPrepper():
    def __init__(self, path='data/kalshi_reddit_sentiment_combined_15min.pkl', target_col='price_close_next', oai_vader_only=None, lookback=12):
        ...
        # get back full test set
        self.X_test_full, self.y_test_full = self._recombine_batches(ttv='test', full_epoch=True)
        self.X_train_full, self.y_train_full = self._recombine_batches(ttv='train', full_epoch=True)
        self.X_val_full, self.y_val_full = self._recombine_batches(ttv='val', full_epoch=True)
    ...
    def batch_generator(self, batch_size=32, ttv='train', change_only=False, full_epoch=False):    
        if ttv=='train':
            data = self.train_df
        elif ttv=='val':
            data = self.val_df
    ...
    def _recombine_batches(self, batch_size=32, ttv='train', change_only=False, full_epoch=False):
        """
        ...
        X_batches = []
        y_batches = []
        for Xb, yb in self.batch_generator(batch_size=batch_size, ttv=ttv, change_only=change_only, full_epoch=full_epoch):
            X_batches.append(Xb)
            y_batches.append(yb)
